...
func carryVersions(a, b *Godeps) error {
	for i, d := range b.Deps {
		for _, da := range a.Deps {
			switch {
			case d.ImportPath == da.ImportPath:
				b.Deps[i].Rev = da.Rev
				b.Deps[i].Comment = da.Comment
			case strings.HasPrefix(d.ImportPath, da.ImportPath+"/"):
				if da.Rev != d.Rev {
					return &revError{d.ImportPath, d.Rev, da.Rev}
				}
			case strings.HasPrefix(da.ImportPath, d.root+"/"):
				if da.Rev != d.Rev {
					return &revError{d.ImportPath, d.Rev, da.Rev}
				}
			}
